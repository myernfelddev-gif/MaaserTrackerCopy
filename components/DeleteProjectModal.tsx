...
const DeleteProjectModal: React.FC<DeleteProjectModalProps> = ({ isOpen, onClose, projectId, projectName }) => {
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title="מחיקת פרויקט">
      <div className="space-y-6 text-center">
        ...
        <div className="space-y-2">
          ...
          <p className="text-slate-500 leading-relaxed">
            האם אתה בטוח שברצונך למחוק את הפרויקט <span className="font-black text-slate-800">"{projectName}"</span>? 
        ...
        {error && (
          <div className="p-4 bg-red-50 border border-red-100 rounded-2xl flex items-center gap-3 text-red-600 text-sm font-bold animate-in fade-in">
            <AlertCircle size={18} />
            <span>{error.message || 'שגיאה במחיקת הפרויקט.'}</span>
          </div>
        )}
